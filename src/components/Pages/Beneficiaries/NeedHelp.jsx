  ...
  return (
    <>
      <div className='space-y-10'>
        <h3 className='font-extrabold md:text-xl  leading-none uppercase'>Нужна помощь?</h3>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mt-10 md:mt-15 lg:mt-18">
        {cardsData.map((card) => (
          <div key={card.id} className="bg-light-gray rounded-2xl lg:rounded-3xl flex flex-col gap-y-10  lg:gap-y-25 px-4 py-6">
            <h5 className="text-[32px]  leading-none">{card.number}</h5>
            <p className="font-medium text-sm md:text-base  leading-none line-clamp-2">{card.text}</p>
          </div>
        ))}
      </div>
    </>
